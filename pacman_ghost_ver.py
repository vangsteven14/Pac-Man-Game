...
spooked_img = pygame.transform.scale(pygame.image.load(f'assets/ghost_images/powerup_pacman.png'), (45, 45))
...
# Initial pacman as ghost animation variables
player_frame = 0
...
class Ghost:
    ...
    def draw(self):
        # Use Pac-Man animation frame for all ghosts
        img = self.img
...
def draw_misc():
    ...
    for i in range(lives):
        screen.blit(pygame.transform.scale(blinky_img, (30, 30)), (650 + i * 40, 915)) # modify this for ghost blinky
...
# Play as blinky the blue ghost
def draw_player():
    # RIGHT = 0, LEFT = 1, UP = 2, DOWN = 3
    if direction == 0:  
        screen.blit(blinky_img, (player_x, player_y))
    elif direction == 1:  
        screen.blit(blinky_img, (player_x, player_y))
    elif direction == 2:  
        screen.blit(blinky_img, (player_x, player_y))
...
while run:
    ...
    if powerup and power_counter < 600:
        ...
        power_counter = 0
        powerup = False
        eaten_ghost = [False, False, False, False] # Reset eaten ghosts
    ...
    blinky = Ghost(blinky_x, blinky_y, targets[0], ghost_speeds[0], player_images[player_frame], blinky_direction, blinky_dead,
                   blinky_box, 0)
    inky = Ghost(inky_x, inky_y, targets[1], ghost_speeds[1], player_images[player_frame], inky_direction, inky_dead,
                 inky_box, 1)
    pinky = Ghost(pinky_x, pinky_y, targets[2], ghost_speeds[2], player_images[player_frame], pinky_direction, pinky_dead,
                  pinky_box, 2)
    clyde = Ghost(clyde_x, clyde_y, targets[3], ghost_speeds[3], player_images[player_frame], clyde_direction, clyde_dead,
                  clyde_box, 3)
    ...
    if moving:
        ...
        if (direction == 0 and turns_allowed[0]) or \
            (direction == 1 and turns_allowed[1]) or \
            (direction == 2 and turns_allowed[2]) or \
            (direction == 3 and turns_allowed[3]):
            player_anim_counter += 1
        if player_anim_counter >= 5:  # Adjust for animation speed
